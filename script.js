...
// === БАЗА ДАННЫХ УСЛУГ (ПОЛНЫЕ ТЕКСТЫ) ===
const services = [
    { 
        id: 1, 
        name: 'Обложка товара', 
        ...
        description: 'Профессиональное оформление главной фотографии товара для повышения CTR.',
        features: [
            'Разработка композиции',
            'Ретушь и цветокоррекция',
            'ИИ-генерация товара в интерьере',
            'Создание фона',
            'Адаптация под мобильные устройства',
            'До 2 правок'
        ],
        timeline: '2 дня',
        deliverable: '1 изображение в формате JPG'
    },
    { 
        id: 2, 
        name: 'Инфографика', 
        ...
        description: 'Визуализация преимуществ товара для быстрого восприятия покупателем.',
        features: [
            'Визуализация преимуществ товара',
            'Создание иконок и схем',
            'Акценты на ключевых характеристиках',
    ...
    { 
        id: 3, 
        name: 'SEO-описание товара', 
        price: 2500, 
        type: 'text',
        image: 'images/services/cover-3.jpg',
        description: 'Продающий текст с ключевыми словами для поднятия карточки в поиске.',
        features: [
            'Подбор ключевых слов (до 10 запросов)',
            'Структурированный текст с абзацами',
            'Уникальность 90%',
            'Соответствие требованиям маркетплейса',
            'До 1500 знаков (оптимально для Озон и ВБ)',
            'До 2 правок'
        ],
        timeline: '1 день',
        deliverable: 'Готовый текст в формате TXT'
    },
    { 
        id: 4, 
        name: 'Видеообложка', 
        ...
        description: 'Динамичное видео для привлечения внимания в ленте выдачи.',
        features: [
            'ИИ-генерация сцен',
            'Монтаж динамичного видео',
            'Демонстрация товара с разных ракурсов',
            'Наложение субтитров',
            'Фоновая музыка и озвучка (опционально)',
            'До 2 правок'
        ],
        timeline: '3 дня',
        deliverable: 'Видео до 20 секунд в формате mp4'
    },
    { 
        id: 5, 
        name: 'Рич-контент', 
        ...
        description: 'Яркая витрина внутри карточки товара для повышения доверия.',
        features: [
            'Разработка структуры блока',
            'Текст + визуальные элементы',
            'Адаптивная вёрстка под маркетплейс',
            'До 2 правок'
        ],
        timeline: '2 дня',
        deliverable: '2 изображения (для ПК и мобильной версии) одного блока в формате JPG'
    },
    { 
        id: 6, 
        name: 'Аудит карточки товара', 
        price: 3000, 
        type: 'audit',
        image: 'images/services/cover-6.jpg',
        description: 'Полный разбор текущей карточки с рекомендациями по росту продаж.',
        features: [
            'Анализ текущего оформления',
            'Проверка читаемости на мобильных',
            'Соответствие требованиям площадки',
            'Рекомендации по улучшению конверсии',
            'Отчёт с примерами улучшений'
        ],
        timeline: '2 дня',
        deliverable: 'Презентация в формате PDF'
    },
    { 
        id: 7, 
        name: 'Пакет «Старт»', 
        ...
        features: [
            'Полное оформление одной карточки',
            'Обложка товара',
            '3 слайда инфографики',
            'SEO-описание (до 1500 знаков)',
            '2 блока рич-контента',
            'До 2 правок'
        ],
        timeline: '4 дня',
        deliverable: '8 изображений в формате JPG, описание в TXT и рекомендации'
    },
    { 
        id: 8, 
        name: 'Пакет «Оптима»', 
        ...
        description: 'Комплексное оформление пяти карточек в едином стиле.',
        features: [
            'Полное оформление пяти карточек',
            'Обложки товаров',
            '3 слайда инфографики для каждого товара',
            'SEO-описание (до 1500 знаков) для каждого товара',
            '2 блока рич-контента для каждого товара',
            'До 4 правок'
        ],
        timeline: '15 дней',
        deliverable: '40 изображений в формате JPG, описание в TXT и рекомендации'
    },
    { 
        id: 9, 
        name: 'Пакет «Масштаб»', 
        ...
        description: 'Масштабное оформление восьми карточек для заполнения всего магазина.',
        features: [
            'Полное оформление восьми карточек',
            'Обложки товаров',
            '3 слайда инфографики для каждого товара',
            'SEO-описание (до 1500 знаков) для каждого товара',
            '2 блока рич-контента для каждого товара',
        ...
        timeline: '24 дня',
        deliverable: '64 изображения в формате JPG, описание в TXT и рекомендации'
    }
];
...
async function checkout() {
    ...
    try {
        ...
        closeCart();
        renderServices(); // Обновляем кнопки
        updateCartUI();
    } catch (e) {
...
function initIndexPage() {
    ...
    if (!container) return; // Если мы не на главной, выходим
...
function toggleCart(id) {
    ...
    localStorage.setItem('cart', JSON.stringify(cart));
    // Если мы на главной - обновляем вид кнопок
    if(document.getElementById('services-container')) {
        initIndexPage();
    } else {
        // Если мы на странице услуги
        renderServicePage();
    }
    updateCartUI();
}
...
function renderServicePage() {
    ...
    const portfolioLink = service.isPackage ? 'portfolio.html#port-complex' : `portfolio.html#port-${service.type}`;

    container.innerHTML = `
        <div class="detail-header">
            <a href="index.html" style="display:inline-block; margin-bottom:20px; color:var(--text-sec);">← Все услуги</a>
            <h1 style="font-size:36px; margin-bottom:10px;">${service.name}</h1>
            <div class="detail-price">${service.price.toLocaleString()} ₽</div>
            <p style="color:var(--text-sec); font-size:18px;">${service.description}</p>
        </div>

        <div class="detail-grid">
            <div class="detail-main">
                <div class="detail-image" style="background-image: url('${service.image || ''}'); background-size: cover; background-position: center;"></div>
                ...
                <div class="detail-section">
                    <h3>Примеры работ:</h3>
                    <a href="${portfolioLink}" style="color:var(--struct); text-decoration:underline;">Смотреть портфолио раздела →</a>
                </div>
            </div>
